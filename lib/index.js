    ...
    const handleMessage = async message => {
      ...
      if (attributeValue !== undefined) {
        if(message.MessageAttributes &&
          message.MessageAttributes['beanstalk.sqsd.task_name'] &&
          message.MessageAttributes['beanstalk.sqsd.task_name'].StringValue === attributeValue
        ) {
          console.log('Message attribute matches provided value', message.MessageAttributes['beanstalk.sqsd.task_name'].StringValue);
        } else {
          console.log('Message attribute does not match provided value. Ignoring message');
          reject(new Error('Message attribute does not match provided value. Ignoring message'));
          return;
        }
      }
